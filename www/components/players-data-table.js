class PlayersDataTable extends HTMLElement {
  constructor() {
    super();
  ...
  connectedCallback() {
    ...
    const userIdFromQuery = (params.get("userid") || "").trim();

    this.render();

    if (userIdFromQuery) {
      const badge = this.shadowRoot.getElementById("userBadge");
      if (badge) {
        badge.textContent = userIdFromQuery;
  ...
  render() {
    this.shadowRoot.innerHTML = `
      ...
      <style>
        ...
        .player-link, .vps-link {
          color: var(--neon-cyan);
          text-decoration: none;
        }

        .player-link:hover, .vps-link:hover {
      ...
      <div id="rowDialog" class="row-dialog" hidden>
        <div class="row-dialog-panel" role="dialog" aria-modal="true" aria-labelledby="rowDialogTitle">
          <div class="row-dialog-header">
            <h3 id="rowDialogTitle" class="row-dialog-title">Player Details</h3>
            <button id="rowDialogClose" class="row-dialog-close" type="button" aria-label="Close player details">×</button>
          </div>
  ...
  renderFilters() {
    ...
    const searchableCols = orderedCols.filter((col) =>
      ["userId", "initials"].includes(col.key),
    );
  ...
  attachEventListeners() {
    ...
    thead.addEventListener("click", (e) => {
      ...
      if (this.sortBy === sortKey) {
        ...
        this.sortBy = sortKey;
        this.sortOrder =
          sortKey === "userId" || sortKey === "initials" ? 1 : -1;
      }
  ...
  async loadData() {
    ...
    try {
      ...
      if (this.sortBy) {
        ...
        params.append(
          "sort_order",
          Number(this.sortOrder) === 1 ? "asc" : "desc",
        );
      }

      if (this.searchTerm) {
        params.append("userId", this.searchTerm);
      }

      const apiUrl =
        this.getAttribute("api-url") || "/api/v1/users-plus/search";
  ...
  renderRows() {
    ...
    if (this.items.length === 0 && !this.loading) {
      tbody.innerHTML = `
        <tr>
          <td colspan="${this.columns.length}" class="empty-state">No players found</td>
        </tr>
      `;
      return;
    }
    ...
    for (let i = startIndex; i < this.items.length; i++) {
        ...
        if (userId) {
          if (document.startViewTransition) {
            document.startViewTransition(() => {
              window.location.href = `/players?userid=${userId}`;
            });
          } else {
      ...
      columns.forEach((col) => {
        ...
        if (col.key === "runTimeTotal") {
          ...
          td.innerHTML = this.renderNameCell(item, value);
        } else if (col.key === "lastSyncAt") {
  ...
  renderNameCell(item, value) {
    const label = this.escapeHtml(this.formatValue(value, "userId"));
    const playerUrl = `/players?userid=${encodeURIComponent(item.userId || "")}`;
    return `
      <div class="row-name-cell">
        <span class="row-name-content">
          <a href="${playerUrl}" class="player-link">${label}</a>
        </span>
      </div>
    `;
  }
...
customElements.define("players-data-table", PlayersDataTable);
