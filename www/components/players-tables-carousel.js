class PlayersTablesCarousel extends HTMLElement {
  constructor() {
    super();
  ...
  async connectedCallback() {
    ...
    this.userId = urlParams.get("userid") || null;
    if (!this.userId) {
      return;
    }

    this.shelfTitle = this.getAttribute("title") || "";
    this.shelfStat = this.getAttribute("shelf-stat") || "";
    this.apiUrl = this.getAttribute("api-url") || this.apiUrl;

    switch(this.shelfStat) { 
      case "runTime": this.sortBy = "runTimeTotal"; break;
      case "startCount": this.sortBy = "startCountTotal"; break;
      case "lastRun": this.sortBy = "lastRun"; break;
      case "createdAt": this.sortBy = "firstSeenAt"; break;
    }
  ...
  async loadData() {
    ...
    try {
      ...
      const fullUrl = this.apiUrl.startsWith("http")
        ? this.apiUrl
        : `${typeof API_BASE !== "undefined" ? API_BASE : ""}${this.apiPath}${this.userId}${this.apiUrl}`;
      ...
      card.innerHTML = `
        <a href="${tableUrl}" class="card-link">
          <div class="image-container">
            <img src="${bgUrl}" alt="${this.escapeHtml(item.vpsdb.name)}" loading="lazy" onerror="this.src='/img/logo.png'">
          </div>
        </a>
        <div class="card-info">
          <div class="info-left">
            <a href="${tableUrl}" class="name-link" title="${this.escapeHtml(item.vpsdb.name)}">${this.escapeHtml(item.vpsdb.name)}</a>
            <div class="row-2">
              ${this.escapeHtml(item.vpsdb.manufacturer || "Unknown")}, ${this.escapeHtml(item.vpsdb.year || "N/A")}
            </div>
  ...
  formatStat(item) {
    ...
    switch (this.shelfStat) {
      ...
      case "lastRun":
        return `<span class="stat-label">Last Played</span><span class="stat-value stat-date">${new Date(val).toLocaleDateString()}</span>`;
      case "startCount":
        return `<span class="stat-label">Plays</span> <span class="stat-value">${val || 0}</span>`;
      case "createdAt":
...
customElements.define("players-tables-carousel", PlayersTablesCarousel);
