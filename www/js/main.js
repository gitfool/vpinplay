...
async function loadAllTablesPage() {
  const result = await api(
    `/api/v1/tables?limit=${ALL_TABLES_PAGE_SIZE}&offset=${allTablesOffset}`,
  );
  const items =
    result.ok && Array.isArray(result.data?.items) ? result.data.items : [];
  ...
  q("allTablesPageInfo").textContent =
    `Page ${pageNumber} (${start}-${end} of ${total})`;
...
async function loadTopPlayerActivity(
  metric,
  days = TOP_PLAYER_DAYS,
  ...
  const res = await api(
    `/api/v1/users/top-activity?metric=${encodeURIComponent(metric)}&days=${encodeURIComponent(days)}&limit=${encodeURIComponent(safeLimit)}`,
  );
  return res.ok && Array.isArray(res.data?.items) ? res.data.items : [];
}

function parseDashboardLimit() {
  const input = q("limitInput");
  if (!input) return 5;
...
async function fetchPaginatedRows(basePath, requestedLimit) {
  ...
  while (remaining > 0) {
    ...
    const res = await api(
      `${basePath}${joiner}limit=${encodeURIComponent(pageLimit)}&offset=${encodeURIComponent(offset)}`,
    );
